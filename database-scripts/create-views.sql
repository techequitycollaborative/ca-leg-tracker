...
CREATE VIEW [LEGTRACKER_SCHEMA].bill_latest_actions AS
SELECT b.bill_id,
    bd.dashboard_id,
    f.first_date,
    ...
    u.user_date,
    u.user_text
FROM [LEGTRACKER_SCHEMA].bill b
JOIN [LEGTRACKER_SCHEMA].bill_dashboard bd USING(bill_id)
LEFT JOIN (
    ...
    FROM (
        ...
        FROM [LEGTRACKER_SCHEMA].bill_history bh
        LEFT JOIN [LEGTRACKER_SCHEMA].chamber c USING(chamber_id)
    ) a
    WHERE a."row" = 1
) h USING (bill_id)
LEFT JOIN (
    ...
    FROM (
        ...
        FROM [LEGTRACKER_SCHEMA].bill_schedule
    ) a
    WHERE a."row" = 1
) s USING (bill_id)
LEFT JOIN (
    ...
    FROM (
        ...
        FROM [LEGTRACKER_SCHEMA].user_action ua
        JOIN [LEGTRACKER_SCHEMA].user_action_type uat USING(user_action_type_id)
    ) a
    JOIN [LEGTRACKER_SCHEMA].bill_dashboard b USING(bill_dashboard_id)
    WHERE a."row" = 1
